...
namespace MQTT {
    ...
    public:
        ...
        ConnectAck(uint8_t *data, size_t length);
    };
    ...
    private:
        ...
        Publish(String topic, uint8_t *payload, size_t length, bool mine) :
                Message(MQTTPUBLISH),
                _topic(topic),
    ...
    public:
        ...
        Publish(String topic, uint8_t *payload, size_t length) :
        ...
        friend Publish Publish_P(String topic, PGM_P payload, size_t length);

        // Construct from a network buffer
        Publish(uint8_t flags, uint8_t *data, size_t length);
        ...
        size_t payload_len(void) const { return _payload_len; }
    ...
    public:
        // Construct from a network buffer
        SubscribeAck(uint8_t *data, size_t length);

        ~SubscribeAck();

        // Get the number of return codes available
        size_t num_rcs(void) const { return _num_rcs; }
    ...
    public:
        // Construct from a network buffer
        UnsubscribeAck(uint8_t *data, size_t length);
