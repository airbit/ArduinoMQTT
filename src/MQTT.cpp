...
namespace MQTT {
    ...
    bool Message::send(WiFiClient &wclient) {
        ...
        while (sent < real_len) {
            count = min(real_len - sent, MQTT_SEND_BLICK_SIZE);
            sent += (ret = wclient.write(const_cast<const uint8_t *>(real_packet + sent), count));
            if (ret != count) break;
        }
    ...
    Message *readPacket(WiFiClient &client) {
        ...
        Serial.print("Receiving mqtt packet: ");
        Serial.println(type);
    ...
    bool Connect::write_variable_header(uint8_t *buf, size_t &bufpos) {
    ...
    bool Connect::write_payload(uint8_t *buf, size_t &bufpos) {
    ...
    // ConnectAck class
    ConnectAck::ConnectAck(uint8_t *data, size_t length) :
            Message(MQTTCONNACK) {
        size_t pos = 0;
    ...
    Publish Publish_P(String topic, PGM_P payload, size_t length) {
        uint8_t *p = new uint8_t[length];
        memcpy_P(p, payload, length);
        return Publish(topic, p, length, true);
    }

    Publish::Publish(uint8_t flags, uint8_t *data, size_t length) :
            Message(MQTTPUBLISH, flags),
            _payload(NULL), _payload_len(0),
            _payload_mine(false) {
        Serial.println("Creating Publish");
        size_t pos = 0;
        _topic = read<String>(data, pos);
        if (qos() > 0)
            _packet_id = read<uint16_t>(data, pos);

        Serial.println("Readed Topic");
        ...
        Serial.println("Created Publish");
    }
    ...
    bool Subscribe::write_variable_header(uint8_t *buf, size_t &bufpos) {
        write_packet_id(buf, bufpos);
    }

    bool Subscribe::write_payload(uint8_t *buf, size_t &bufpos) {
        write(buf, bufpos, _buffer, _buflen);
    }


    // SubscribeAck class
    SubscribeAck::SubscribeAck(uint8_t *data, size_t length) :
            Message(MQTTSUBACK),
            _rcs(NULL) {
        size_t pos = 0;
        _packet_id = read<uint16_t>(data, pos);
    ...
    bool Unsubscribe::write_variable_header(uint8_t *buf, size_t &bufpos) {
        write_packet_id(buf, bufpos);
    }

    bool Unsubscribe::write_payload(uint8_t *buf, size_t &bufpos) {
        write(buf, bufpos, _buffer, _buflen);
    }


    // SubscribeAck class
    UnsubscribeAck::UnsubscribeAck(uint8_t *data, size_t length) :
